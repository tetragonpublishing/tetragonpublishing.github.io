...
function loadScript() {
  ...
  script.src = "http://maps.googleapis.com/maps/api/js?key=AIzaSyBmNn6HldGUbESi0i0ubVWWJ3bbrGuT2Wc&sensor=true&callback=initialize";
...
$(document).ready(function(){
	  ...
	  $("#contact_submit").click(function() {
	    if ($('#contact-form').validationEngine('validate')) {
          $('#contact_submit').hide();
	      $('#contact_loader').show();
	      $.post("/m", $('#contact-form').serialize(), function(data) {
	    	  if (!data.success) {
	    		  alert(data.errors);
